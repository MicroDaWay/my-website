...
![branch01.png](../static/img/branch01.png)
![branch02.png](../static/img/branch02.png)
![branch03.png](../static/img/branch03.png)
...
1. 静态页面的分支必须叫做：`gh-pages`
2. 如果希望页面可以通过 xxx.github.io 访问，则需要将 github 的仓库名配置为 xxx.github.io

## docusaurus

facebook 推出的开源静态内容管理系统，通过它可以快速的部署一个静态网站
...
- 部署项目到 github：`npm run deploy` 或 `yarn deploy`
- **上传代码时，两个注意事项：**

  1. 使用 deploymentBranch 来指定分支：`deploymentBranch: 'gh-pages'`
  2. 还需要配置一个环境变量：`GIT_USER: github 的用户名`

- 配置项目：`docusaurus.config.js` 是项目的配置文件
- 添加页面：在 docusaurus 框架中，页面分成三种：page、blog、doc
